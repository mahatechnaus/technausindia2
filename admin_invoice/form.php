...
?>
                                    <option value="" hidden>Choose here</option>
                                    <option value="<?php echo $row_ah['buyer_id']; ?>">
                                        <?php echo $row_ah['company_name']; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
...
?>
                                    ...
                                    <option value="<?php echo $row_ah['product_description']; ?>">
                                        <?php echo $row_ah['product_description']; ?></option>
                                    <?php } ?>
                                </select><br><br>
                            ...
                            <button type="button" class="btn btn-primary" onclick="clickFunc()" id="clone"> Add Product
                                2</button>
...
?>
    ...
    $(document).ready(function() {
        ...
        $('#clone').click(function() {
            ...
            $(newel).find('textarea:nth-child(13)').attr("id", "desc_" + index).val("");
            ...
            $(newel).find('textarea:nth-child(13)').attr("name", "desc[" + index + "]");
            ...
            var $this = $(this);
            $this.text('Add Product ' + (count + 1));

            $(newel).find('input:nth-child(9)').attr("required", false);
    ...
    function clickFunc() {
        ...
        var btn1 = document.querySelector('#remove');
        if (count >= 10) { // placed inside the click function
            btn.disabled = true;
        }
        if (count > 1) {
            btn1.disabled = false;
    ...
    $(document).on('click', '#remove', function() {
        ...
        count -= 1;
        $('#clone').text('Add Product ' + (count + 1));
        if (count > 1) {
    ...
    function fetch(x) {
        ...
        if (doc == "") {
            ...
            document.getElementById('qty_' + x).value = "";
        } else {
            $.ajax({
                url: "check2.php",
                method: "POST",
